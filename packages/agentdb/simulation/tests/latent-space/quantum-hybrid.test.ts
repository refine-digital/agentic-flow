...
interface ResourceRequirements {
  avgQubitsRequired: number;
  maxGateDepth: number;
  [key: string]: unknown;
}

interface TheoreticalSpeedups {
  maxTheoreticalSpeedup: number;
  [key: string]: unknown;
}
...
describe('QuantumHybrid (Theoretical)', () => {
  ...
  describe('2025 Viability (Current)', () => {
    it('should assess near-term viability ~12.4%', () => {
      const nearTerm = report.summary.nearTermViability as number;
    ...
    it('should have limited qubit count', () => {
      const hardware2025 = (quantumHybridScenario.config.hardwareProfiles as HardwareProfile[]).find(h => h.year === 2025);
  ...
  describe('2030 Viability (Mid-term)', () => {
    it('should project ~38.2% viability', () => {
      const results = (report.detailedResults as DetailedResult[]).filter(r => r.hardwareYear === 2030);
  ...
  describe('2040 Viability (Long-term)', () => {
    it('should project ~84.7% viability', () => {
      const longTerm = report.summary.longTermProjection as number;
    ...
    it('should have advanced hardware', () => {
      const hardware2040 = (quantumHybridScenario.config.hardwareProfiles as HardwareProfile[]).find(h => h.year === 2040);
  ...
  describe('Grover Algorithm', () => {
    it('should test Grover search', () => {
      const algorithms = quantumHybridScenario.config.algorithms as AlgorithmConfig[];
      const grover = algorithms.find(a => a.name === 'grover');
      expect(grover).toBeDefined();
    });

    it('should calculate sqrt(M) speedup', () => {
      const groverResults = (report.detailedResults as DetailedResult[]).filter(r => r.algorithm === 'grover');

      groverResults.forEach(r => {
        if (r.speedups?.groverSpeedup) {
          const M = r.parameters?.neighborhoodSize || 16;
          expect(r.speedups.groverSpeedup).toBeCloseTo(Math.sqrt(M as number), 2);
        }
      });
    });

    it('should target 4x speedup for M=16', () => {
      const grover16 = (report.detailedResults as DetailedResult[]).find(
  ...
  describe('Quantum Walk', () => {
    ...
    it('should calculate speedup', () => {
      const qwalkResults = (report.detailedResults as DetailedResult[]).filter(r => r.algorithm === 'quantum-walk');
  ...
  describe('Amplitude Encoding', () => {
    ...
    it('should require log(d) qubits', () => {
      const ampResults = (report.detailedResults as DetailedResult[]).filter(r => r.algorithm === 'amplitude-encoding');
  ...
  describe('Hybrid Approach', () => {
    ...
    it('should split workload', () => {
      const hybridResults = (report.detailedResults as DetailedResult[]).filter(r => r.algorithm === 'hybrid');
    ...
    it('should respect qubit budget', () => {
      const hybrid = (quantumHybridScenario.config.algorithms as AlgorithmConfig[]).find(a => a.name === 'hybrid');
  ...
  describe('Resource Requirements', () => {
    it('should track qubit requirements', () => {
      const metrics = report.metrics.resourceRequirements as ResourceRequirements;
  ...
  describe('Hardware Progression', () => {
    it('should test multiple hardware profiles', () => {
      const profiles = quantumHybridScenario.config.hardwareProfiles as HardwareProfile[];
  ...
  describe('Theoretical Speedup Validation', () => {
    it('should calculate theoretical speedups', () => {
      const speedups = report.metrics.theoreticalSpeedups as TheoreticalSpeedups;
  ...
  describe('Classical Baseline', () => {
    ...
    it('should have 1x speedup for classical', () => {
      const classicalResults = (report.detailedResults as DetailedResult[]).filter(r => r.algorithm === 'classical');
